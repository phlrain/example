...
class LSTMModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--use-basic', default=False, action='store_true',
                            help='use baisc lstm' )
    ...
    def build_model(cls, args, task):
        ...
        print( "use basic", args.use_basic )
...
class LSTMEncoder(FairseqEncoder):
    ...
    def __init__(
        ...
        if not use_basic:
            ...
            self.lstm = BasicLSTM( embed_dim, hidden_size, num_layers, dropout=self.dropout_out if num_layers > 1 else 0.)
    ...
    def forward(self, src_tokens, src_lengths):
        ...
        if not self.use_basic:
            ...
            c0 = x.new_zeros(*state_size)
            encoder_padding_mask = src_tokens.eq(self.padding_idx).t()
            x, last_state = self.lstm( x, (h0, c0), encoder_padding_mask.float() )
            final_hiddens, final_cells = last_state
...
class BasicLSTMCell(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        def step( step_in, pre_hidden, pre_cell ):
            concat_in = torch.cat( [step_in, pre_hidden], dim=1)
            gate_input = self.linear( concat_in )
        ...
        new_hidden, new_cell = step( input, pre_hidden, pre_cell )
...
class BasicLSTM(nn.Module):
    ...
    def __init__( self, input_size, hidden_size, num_layers, dropout=0.0):
        ...
        self.linear_arra = nn.ModuleList( [ nn.Linear( (hidden_size + input_size) if i == 0 else ( hidden_size * 2),  hidden_size * 4)  for i in range(num_layers)] )
    ...
    def forward(self, input, hidden, enc_len_mask):
        ...
        for j in range(seq_len):
            step_mask = enc_len_mask[j]
            step_mask = step_mask[:, None]
            step_in = input[j]
            for i in range(self.num_layers):
                new_hidden, new_cell = step( step_in, pre_hidden_array[i], pre_cell_array[i], i)
                
                new_hidden = new_hidden * step_mask +  pre_hidden_array[i] * ( 1 - step_mask )
                neW_cell = new_cell * step_mask +  pre_cell_array[i] * ( 1 - step_mask )
                pre_hidden_array[i] = new_hidden
                pre_cell_array[i] = new_cell
                
                step_in = new_hidden

                if j != self.num_layers and self.dropout > 0.0:
                    step_in = F.dropout(step_in, p=self.dropout)
